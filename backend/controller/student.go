...
func CreateStudent(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", students.EmployeeID).First(&employee); tx.RowsAffected == 0 {
...
func ListStudent(c *gin.Context) {
	...
	if err := entity.DB().Preload("Gender").Preload("Program").Preload("Province").Preload("Employee").Raw("SELECT * FROM students").Find(&students).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
